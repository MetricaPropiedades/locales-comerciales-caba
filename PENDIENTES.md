# PENDIENTES para aplicar en "Locales Comerciales CABA.html"

Última actualización: 17/09/2026.

## TODO APLICADO (17/09)
No queda nada pendiente. Se aplicaron: las 3 ediciones de texto (nota de brokers en Rappi, "8 cocheras" en OSDE Ambulancias, City Bell + Av. 8/12 de La Plata en Farmacias Simplicity), el rename Valeria -> Romina, y las 5 pestañas nuevas (Hazrat Namasté India, Marca Nueva Sabores Express, Sabores Express, Hamburguesas Extremas, Laura Flores).

Nota de método para Sabores Express y Hamburguesas Extremas (confirmado por Juan el 17/09): NO son esquinas ni polígonos. Vale cualquier local sobre la calle indicada dentro del rango de alturas de la ficha.

Desde el 17/09 las corridas diarias ya no usan Cowork ni el sandbox: corren en Claude Code con git+node (ver legacy-tareas/ADAPTACION-CLAUDE-CODE.md). Si en el futuro queda algo sin aplicar, anotarlo acá.
